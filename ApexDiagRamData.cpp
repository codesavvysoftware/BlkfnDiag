...
#include "ApexDiagnostic.hpp"                   // for TEST_LOOP_COMPLETE etc.
#include "ApexDiagRamData.hpp"                  // for class definiton
#include "ApexDiagSafeRam.hpp"                  // for DgnSafeRam

// FORWARD REFERENCES
// Main testing function, written in assembler.
extern "C" UINT32 ApexDiagRamDataRunTestAsm(UINT32 numwords, UINT32 *RAMAddr, UINT32 *shadowRam);
...
void ApexDiagRamData::Init()
{
    ...
    UINT32 startTime = HI_ApexReg.SystemTime;
    (void)RunTest(NULL);
    UINT32 overheadTime = HI_ApexReg.SystemTime - startTime;
    ...
    if (ApexDiagnostic::SLICE_TIME_LIMIT_US <= overheadTime)
    {
    ...
    else 
    {
        ...
        (void)RunTest(NULL);
        UINT32 measureDelta = HI_ApexReg.SystemTime - startTime;
        if (irqEnabled)
        {
            HI_EnableIrq();                    // Enable interrupts
        }
        ...
        else
        {
            ...
            // measurements
            iterationsPerSlice = blksToChk * (ApexDiagnostic::SLICE_TIME_LIMIT_US - overheadTime)
...
/////////////////////////////////////////////////////////////////////////////
ApexDiagnostic::TestState ApexDiagRamData::RunTest(ApexDiagnostic::DiagControlBlock *)
{
    ...
    if (blksToRamEnd > blksToChk)
    {
        ...
        // end of memory
        (void)ApexDiagRamDataRunTestAsm(blksToChk, (UINT32 *) curAddr, (UINT32 *) shadowRamAddr);
    ...
    else
    {
        ...
        // end of memory
        (void)ApexDiagRamDataRunTestAsm(blksToRamEnd, (UINT32 *) curAddr, (UINT32 *) shadowRamAddr);
